...
public class SDKMgr : Singleton<SDKMgr>, ISDKCallback
{
    ...
    static bool bIpayInit = false;
    void initIPaySDK()
    {
        //add by richard
        try
        {
            Debug.Log("want init ipaysdk");
            AndroidJavaClass jc = new AndroidJavaClass("com.doule.buyu.DouleBuyuPayActivity");
            if (jc == null)
            {
                lblMessage = "AndroidJavaClass is error!\n";
                Debug.Log(lblMessage);
                return;
            }
            else
            {
                lblMessage = "AndroidJavaClass is OK!\n";
                Debug.Log(lblMessage);
                
            }
            Debug.Log("ipaysdk AndroidJavaClass init finish");
            jo = jc.CallStatic<AndroidJavaObject>("currentActivity");
            bIpayInit = true;
        ...
        if (jo == null)
        {
            lblMessage += "AndroidJavaObject is error!\n";
            Debug.Log(lblMessage);
            // GlobalEffectMgr.Instance.ShowMessageBox(lblMessage, MssageType.Exit_Msg_5);
            return;
        }
        else
        {
            lblMessage += "AndroidJavaObject is ok!\n";
            Debug.Log(lblMessage);
        }
    }

    public void startPay(string appuserid,int waresid)
    {
        try
        {
            if(bIpayInit==false)
            ...
            lblMessage += "startPay "+appuserid +"#"+ waresid + "\n";
            jo.Call("startPay",appuserid, waresid);
        }
        catch (UnityException ex)
        {
            lblMessage += ex.Message;
